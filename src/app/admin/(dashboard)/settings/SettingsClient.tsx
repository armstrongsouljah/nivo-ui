...
const inputCls = "w-full bg-zinc-950 border border-zinc-800 text-white text-sm px-3.5 py-2.5 rounded-md placeholder-zinc-600 focus:outline-none focus:border-zinc-500 transition-colors";
const readonlyCls = "w-full bg-zinc-800/40 border border-zinc-800 text-zinc-500 text-sm px-3.5 py-2.5 rounded-md cursor-not-allowed select-all";
  ...
  return (
    <SectionCard title="Profile" icon={<User size={15} />}>
      <div className="space-y-4">
        ...
        {error && (
          <p className="text-[11px] text-red-400 font-semibold bg-red-500/10 px-3 py-2 rounded-md">{error}</p>
        )}

        <div className="flex items-center gap-3 pt-1">
          <button
            onClick={handleSave} disabled={saving}
            className="flex items-center gap-1.5 px-4 py-2.5 bg-white text-black text-xs font-bold uppercase tracking-widest rounded-md hover:bg-zinc-200 disabled:opacity-50 transition-colors"
          >
  ...
  return (
    <SectionCard title="Security" icon={<ShieldCheck size={15} />}>
      <div className="space-y-4">
            ...
            {[8, 12, 16].map((len) => (
              <div
                ...
                className={`h-1 flex-1 rounded-full transition-colors ${
                  next.length >= len ? "bg-green-500" : "bg-zinc-800"
                }`}
              />
            ))}
            <span className="text-[10px] text-zinc-600 ml-2 self-center">
        ...
        {done && (
          <p className="text-[11px] text-green-400 font-semibold bg-green-500/10 px-3 py-2 rounded-md flex items-center gap-1.5">
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
      ...
      <div className="flex flex-col items-center text-center mb-5 pb-5 border-b border-zinc-800">
        ...
        <p className="text-[11px] text-zinc-500 mt-0.5">{profile.email}</p>
        <span className="mt-2 inline-block text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full bg-blue-500/10 text-blue-400">
          {profile.role}
        </span>
      </div>
        ...
        {rows.map((r) => (
          <li key={r.label} className="flex justify-between text-xs">
            ...
            <span className={`font-semibold ${
              r.label === "Status"
                ? profile.is_email_verified ? "text-green-400" : "text-amber-400"
                : "text-white"
            }`}>{r.value}</span>
          </li>
        ))}
...
export default function SettingsClient({ profile: initial }: { profile: AdminProfile | null }) {
  ...
  if (!profile) {
    return (
      <main className="flex-1 px-4 sm:px-6 py-6">
        <p className="text-sm text-zinc-600">Failed to load profile. Please refresh.</p>
      </main>
    );
  }
  ...
  return (
    <main className="flex-1 px-4 sm:px-6 py-6 overflow-y-auto">
      <div className="mb-6">
        <p className="text-[11px] font-bold tracking-[0.2em] text-zinc-500 uppercase mb-1">Account</p>
