...
import type { Category, AttributeDetail } from "@/lib/api";

// ─── useCategories hook ───────────────────────────────────────────────────────
// Initial data is fetched server-side and passed as a prop — no client fetch needed.

function useCategories(initial: Category[]) {
  ...
  return { categories, loading: false, error: null, createCategory };
}

// ─── useAttributes hook ───────────────────────────────────────────────────────

function useAttributes(initial: AttributeDetail[]) {
  const [attributes] = useState<AttributeDetail[]>(initial);
  return { attributes, loading: false };
}
...
export default function NewProductPage({
  ...
  initialAttributes: AttributeDetail[];
}) {
  ...
  const { categories, loading: loadingCategories, createCategory } = useCategories(initialCategories);
