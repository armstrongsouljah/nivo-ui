  ...
  return (
    <form onSubmit={handleSubmit} className="flex items-center gap-2">
      <input
        ...
        className="flex-1 bg-zinc-900 border border-zinc-800 text-white text-sm px-3.5 py-2.5 rounded-md placeholder-zinc-600 focus:outline-none focus:border-zinc-500 transition-colors"
      />
      <button
        type="submit"
        disabled={saving || !name.trim()}
        className="flex items-center gap-1.5 px-4 py-2.5 bg-white text-black text-xs font-bold uppercase tracking-widest rounded-md hover:bg-zinc-200 disabled:opacity-40 transition-colors shrink-0"
      >
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-wrap items-center gap-2 mt-2">
      <input
        ...
        autoFocus
        className="flex-1 min-w-32 bg-zinc-950 border border-zinc-700 text-white text-sm px-3 py-2 rounded-md placeholder-zinc-600 focus:outline-none focus:border-zinc-500"
      />
      <div className="flex items-center gap-1.5 shrink-0">
        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest whitespace-nowrap">
          Hex color
        </label>
        <div className="relative flex items-center">
          {hex && (
            <span
              className="w-5 h-5 rounded-full border border-zinc-600 mr-1.5 shrink-0"
          ...
          <input
            ...
            placeholder="#000000"
            className="w-24 bg-zinc-950 border border-zinc-700 text-white text-sm px-2.5 py-2 rounded-md placeholder-zinc-600 focus:outline-none focus:border-zinc-500"
          />
        </div>
      </div>
      <div className="flex gap-1.5 shrink-0">
        <button
          ...
          className="flex items-center gap-1 px-3 py-2 bg-white text-black text-xs font-bold rounded-md hover:bg-zinc-200 disabled:opacity-40 transition-colors"
        >
      ...
      {error && <p className="w-full text-[11px] text-red-400 font-semibold">{error}</p>}
    </form>
  );
}
    ...
    <div className="border border-zinc-800 rounded-xl bg-zinc-900 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4">
        ...
        <button
          ...
          disabled={deleting}
          className="p-1.5 text-zinc-600 hover:text-red-400 hover:bg-red-500/10 rounded-md transition-colors disabled:opacity-40 ml-2 shrink-0"
            ...
            <div className="flex flex-wrap gap-2">
              {values.map((val) => {
                ...
                return (
                  <div
                    key={val.id}
                    className="flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 bg-zinc-800 rounded-full group"
                  >
                    {hex && (
                      <span
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-0.5 text-zinc-500 hover:text-red-400 transition-all rounded-full"
                      title="Remove"
                    >
                      <X size={11} />
                    </button>
...
export default function AttributesClient({ initial }: { initial: AttributeDetail[] }) {
  ...
  return (
    <main className="flex-1 px-4 sm:px-6 py-6 overflow-y-auto">
      <div className="mb-8">
        <p className="text-[11px] font-bold tracking-[0.2em] text-zinc-500 uppercase mb-1">Catalogue</p>
        <h1 className="text-xl font-black text-white uppercase tracking-tight">Attributes</h1>
      ...
      <div className="max-w-2xl space-y-6">
        ...
        {/* Attribute list */}
        {attributes.length === 0 ? (
          <p className="text-sm text-zinc-600 text-center py-12 border border-dashed border-zinc-800 rounded-xl">
