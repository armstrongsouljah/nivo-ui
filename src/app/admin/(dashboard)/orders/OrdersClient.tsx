...
function StatusBadge({ status }: { status: string }) {
  return (
    <span className={`text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full ${STATUS_STYLES[status] ?? "bg-zinc-800 text-zinc-400"}`}>
      {status}
    </span>
  );
}
...
export default function OrdersClient({ initial }: { initial: OrderSummary[] }) {
  ...
  return (
    <main className="flex-1 px-4 sm:px-6 py-6 overflow-y-auto">
      {/* Header */}
      <div className="mb-6">
        <p className="text-[11px] font-bold tracking-[0.2em] text-zinc-500 uppercase mb-1">Management</p>
        <h1 className="text-xl font-black text-white uppercase tracking-tight">Orders</h1>
        <p className="text-xs text-zinc-500 mt-1">{orders.length} total order{orders.length !== 1 ? "s" : ""}</p>
      </div>
      ...
      <div className="relative mb-4 max-w-sm">
        ...
        <input
          ...
          className="w-full bg-zinc-900 border border-zinc-800 text-white text-sm pl-8 pr-3.5 py-2 rounded-md placeholder-zinc-600 focus:outline-none focus:border-zinc-500 transition-colors"
        />
      </div>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[11px] font-bold uppercase tracking-widest transition-colors ${
                active ? "bg-white text-black" : "bg-zinc-900 text-zinc-400 hover:text-white border border-zinc-800"
              }`}
            >
              {s === "All" ? "All" : s}
              {count > 0 && (
                <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-full ${active ? "bg-black/20 text-black" : "bg-zinc-800 text-zinc-400"}`}>
                  {count}
                </span>
              )}
            </button>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[640px] text-sm">
              ...
              <tbody>
                {filtered.map((order) => (
                  <tr key={order.id} className="border-b border-zinc-800/50 last:border-b-0 hover:bg-zinc-800/30 transition-colors group">
                    ...
                    <td className="py-3.5 px-4">
                      <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-full ${order.is_paid ? "bg-green-500/10 text-green-400" : "bg-zinc-800 text-zinc-500"}`}>
