...
function StatusBadge({ status }: { status: string }) {
  return (
    <span className={`text-[11px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full ${STATUS_STYLES[status] ?? "bg-zinc-800 text-zinc-400"}`}>
      {status}
    </span>
  );
}
...
function SectionCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 sm:p-6">
      <h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-4">{title}</h3>
      {children}
    </div>
  );
}
...
export default function OrderDetailClient({ order: initial }: { order: OrderDetail }) {
  ...
  return (
    <main className="flex-1 px-4 sm:px-6 py-6 overflow-y-auto">
      ...
      <Link
        href="/admin/orders"
        className="inline-flex items-center gap-1.5 text-xs font-bold text-zinc-500 hover:text-white uppercase tracking-widest transition-colors mb-6"
      >
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          {/* <p className="font-mono text-[11px] text-zinc-600">{order.id}</p> */}
          <p className="text-xs text-zinc-500 mt-1">{fmtDate(order.created_at)}</p>
        </div>
        ...
        <div className="space-y-4">
              ...
              <table className="w-full text-sm min-w-[480px]">
                <thead>
                  <tr className="border-b border-zinc-800">
                    {["Item", "Qty", "Unit Price", "Subtotal"].map((h) => (
                      <th key={h} className="text-left text-[10px] font-bold text-zinc-500 uppercase tracking-widest pb-3 pr-4 last:text-right last:pr-0">
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {order.items.map((item) => (
                    <tr key={item.id} className="border-b border-zinc-800/50 last:border-b-0">
                      <td className="py-3 pr-4 text-white font-medium">
                        {item.variant_label || "—"}
                      </td>
                      <td className="py-3 pr-4 text-zinc-400">{item.quantity}</td>
                      <td className="py-3 pr-4 text-zinc-300">{fmtPrice(item.price_at_purchase)}</td>
          ...
          {(transitions.length > 0 || error) && (
            <SectionCard title="Actions">
              {error && (
                <p className="text-[11px] text-red-400 font-semibold mb-4 bg-red-500/10 px-3 py-2 rounded-md">
                  {error}
                </p>
              )}
        ...
        <div className="space-y-4">
          ...
          <SectionCard title="Delivery Details">
            <div className="space-y-2 text-sm">
              ...
              <div>
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-0.5">Phone</p>
                <p className="text-white">{addr?.phone || "—"}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-0.5">Address</p>
          ...
          <SectionCard title="Order Info">
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className={`font-semibold ${order.is_paid ? "text-green-400" : "text-amber-400"}`}>
              ...
              <div className="flex justify-between">
                ...
                <span className="text-zinc-300 text-xs">{fmtDate(order.created_at)}</span>
              </div>
              {order.updated_at !== order.created_at && (
                <div className="flex justify-between">
                  <span className="text-zinc-500">Updated</span>
                  <span className="text-zinc-300 text-xs">{fmtDate(order.updated_at)}</span>
                </div>
              )}
            </div>
          </SectionCard>
