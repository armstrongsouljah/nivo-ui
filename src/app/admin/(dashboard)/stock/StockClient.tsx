  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <input
        ...
        placeholder="±qty"
        className="w-20 bg-zinc-800 border border-zinc-700 text-white text-xs px-2 py-1.5 rounded-md focus:outline-none focus:border-zinc-500"
      />
      <input
        type="text"
        value={notes}
        ...
        className="flex-1 min-w-24 bg-zinc-800 border border-zinc-700 text-white text-xs px-2 py-1.5 rounded-md focus:outline-none focus:border-zinc-500"
      />
      <button onClick={handleSave} disabled={busy}
        className="flex items-center gap-1 px-2.5 py-1.5 bg-white text-black text-xs font-bold rounded-md hover:bg-zinc-200 disabled:opacity-50 transition-colors">
...
export default function StockClient({ initial }: { initial: StockEntry[] }) {
  ...
  return (
    <main className="flex-1 px-4 sm:px-6 py-6 overflow-y-auto">
      {/* Header */}
      <div className="mb-6">
        <p className="text-[11px] font-bold tracking-[0.2em] text-zinc-500 uppercase mb-1">Inventory</p>
        <h1 className="text-xl font-black text-white uppercase tracking-tight">Stock</h1>
        <p className="text-xs text-zinc-500 mt-1">{entries.length} variant{entries.length !== 1 ? "s" : ""} tracked</p>
      </div>

      {/* Summary chips */}
      <div className="flex flex-wrap gap-2 mb-5">
        {[
          { key: "all",  label: "All",      count: entries.length, color: "bg-zinc-900 text-zinc-400 border border-zinc-800" },
          { key: "low",  label: "Low stock", count: lowCount,       color: "bg-amber-500/10 text-amber-400 border border-amber-500/20" },
          { key: "out",  label: "Out",       count: outCount,       color: "bg-red-500/10 text-red-400 border border-red-500/20" },
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[11px] font-bold uppercase tracking-widest transition-colors ${
              filter === key ? "bg-white text-black" : color
            }`}
          >
            {label}
        ...
        <div className="relative ml-auto">
          ...
          <input
            ...
            className="bg-zinc-900 border border-zinc-800 text-white text-xs pl-8 pr-3 py-2 rounded-md w-52 placeholder-zinc-600 focus:outline-none focus:border-zinc-500"
          />
        </div>
      </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          <table className="w-full text-sm min-w-[640px]">
            ...
            <tbody>
              {filtered.map((entry) => (
                <>
                  <tr key={entry.id} className="border-b border-zinc-800/50 last:border-b-0 hover:bg-zinc-800/30 transition-colors">
                    <td className="py-3 px-4 pl-5 font-semibold text-white">{entry.product_name ?? "—"}</td>
                    <td className="py-3 px-4 font-mono text-[11px] text-zinc-500">{entry.variant_sku}</td>
                    <td className="py-3 px-4 text-zinc-400 text-xs">{entry.age_group}</td>
                    <td className="py-3 px-4">
                      <span className={`text-sm font-black ${
                        entry.quantity === 0 ? "text-red-400" :
                        entry.is_low_stock ? "text-amber-400" : "text-white"
                      }`}>{entry.quantity}</span>
                    </td>
                    <td className="py-3 px-4 text-zinc-500 text-xs">{entry.low_stock_threshold}</td>
                    <td className="py-3 px-4">
                      {entry.quantity === 0 ? (
                        ...
                        <span className="text-[10px] font-bold text-green-400 uppercase tracking-wider">In stock</span>
                      )}
                    </td>
                    <td className="py-3 px-4 pr-5 text-right">
                      <button
                        onClick={() => setAdjustingId((prev) => prev === entry.id ? null : entry.id)}
                        className="flex items-center gap-1 text-[11px] font-bold text-zinc-500 hover:text-white transition-colors ml-auto"
                      >
                  ...
                  {adjustingId === entry.id && (
                    <tr key={`${entry.id}-adjust`} className="border-b border-zinc-800/50 bg-zinc-800/20">
